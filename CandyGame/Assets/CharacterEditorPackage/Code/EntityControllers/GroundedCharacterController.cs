...
{
    [SerializeField] float m_WalkForce = 0.0f; // regular move speed
    [SerializeField] float m_WalkForceApplyLimit = 0.0f; // maximum walk force 

    [SerializeField] float m_StoppingForce = 0.0f; //when the player stops moving but slides 
    [SerializeField] bool m_ApplyStoppingForceWhenActivelyBraking = false; // boolean for indicating when to break movement
    ...
    [SerializeField] float m_Gravity = 0.0f; // our own gravity
    ...
    [SerializeField] bool m_AlignRotationToGroundedNormal = false; // what is this???
    //Jumping values
    [SerializeField] float m_JumpVelocity = 0.0f;
    [SerializeField] float m_JumpCutVelocity = 0.0f; //prevent the double jump????

    [SerializeField] float m_MinAllowedJumpCutVelocity = 0.0f; // how much we need to cut off the double jump?
    [SerializeField] float m_GroundedToleranceTime = 0.0f; //what??? 

    [SerializeField] float m_JumpCacheTime = 0.0f; // what??? 
    [SerializeField] float m_JumpAlignedToGroundFactor = 0.0f;// what?? 

    [SerializeField] float m_HorizontalJumpBoostFactor = 0.0f; // what??    
    ...
    protected override void UpdateController()
    {
        ...
        if (isGrounded)
        {
            m_LastGroundedTime = Time.fixedTime; //the previous time we were grounded. 
    ...
    public Vector2 GetGravity()
    {
        ...
        if (m_ApplyGravityIntoGroundNormal)
        {
            ...
            Debug.Log(m_ControlledCollider.GetGroundedInfo().GetNormal());
        }
